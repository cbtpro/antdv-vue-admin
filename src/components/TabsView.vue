...
<template>
  <div class="tabs-view-container" @contextmenu="preventContextMenu">
    <a-tabs
      v-model:activeKey="tabStore.activeTab"
      type="editable-card"
      hide-add
      @tabClick="handleTabClick"
      @edit="(targetKey, action) => action === 'remove' && handleTabClose(targetKey as string)"
    >
      <a-tab-pane
      ...
      >
        <template #tab>
          <div
            class="tab-label"
            @contextmenu.prevent.stop="(e) => showContextMenu(e, tab)"
          >
            {{ tab.title }}
            <push-pin-outlined v-if="tab.fixed" />
          </div>
        </template>
      </a-tab-pane>
    </a-tabs>
    ...
    >
      <a-menu>
        <a-menu-item @click="handleContextMenuClick('closeAll')">
          关闭所有
        </a-menu-item>
        <a-menu-item @click="handleContextMenuClick('closeOthers')">
          关闭其他
        </a-menu-item>
        <a-menu-item @click="handleContextMenuClick('closeRight')">
          关闭右侧
        </a-menu-item>
        <a-menu-item @click="handleContextMenuClick('closeLeft')">
          关闭左侧
        </a-menu-item>
