...
import { computed, onBeforeUnmount, onMounted, ref, watch, type VNode } from 'vue';
import useWebSocket from '@/composables/useWebSocket';

defineOptions({ name: 'PlaygroundIndependent' });

const { status, message, error, send, connect, disconnect } = useWebSocket({
  ...
  return (
    <div style={styles.value.layout}>
      ...
      <div style={styles.value.chat}>
        ...
        <Bubble.List
          items={
            items && items.value && items.value.length > 0
              ? items.value
